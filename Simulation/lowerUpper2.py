...
def simulate(svca, omega, executeServiceLB, q=0.5, eta=0, v=None, debug=False):
    # Similar to lowerUpper.py, but adapted for systems in which events may
    #   occur at noninteger times.
    # Only difference: dictionary of calls (omega.calls) must have keys
    #   based upon the order of arrival (rather than their arrival times)
    bases = svca.getBases()
    calls = omega.getCalls()
    C     = len(calls)
    times = [calls[i]['arr'] for i in range(C)]
    locs  = [calls[i]['loc'] for i in range(C)] 
    svcs  = [calls[i]['svc'] for i in range(C)]
    A     = svca.A
    ...
    felLB.addEvent(T+eps, 'end', priority=-1)

    felUB = FutureEventsList()
    felUB.addEvent(T+eps, 'end', priority=-1)
    ...
    while stateLB['t'] <= T and stateUB['t'] <= T:
        ...
        if eventLB == 'arrival' and eventUB == 'arrival':
            ...
            executeArrivalLB(stateLB, statsLB, calls[c], felLB, svca)
            executeArrivalUB(stateUB, statsUB, calls[c], felUB)
