...
#   Variant of formulation that is more amenable to gradient search.

def solve(svcArea, arrStream, omega, penalty, flag=False, grad=False,\
				search=False,  soln=False):
	    ...
	    for l in xrange(len(steps)):
	        ...
	        if gamma.ndim == 1:
	            for t in xrange(T+1):
	                for j in bases:
	                    if t in calls and j in B[calls[t]['loc']]:
	                        y[t][j].setAttr("obj", current[t]*(RP[t][j]-1))
	                    else:
	                        y[t][j].setAttr("obj", current[t]*RP[t][j])
	            ...
	            for t in xrange(T+1):
	                for j in bases:
	                    if t in calls and j in B[calls[t]['loc']]:
	                        y[t][j].setAttr("obj", current[t][j]*(RP[t][j]-1))
	                    else:
	                        y[t][j].setAttr("obj", current[t][j]*RP[t][j])
