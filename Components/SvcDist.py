...
class SvcDist:
    ...
    def __buildCumul(self):
        # Builds a vector containing the cdf evaulated only at the breakpoints
    ...
    def __buildCDF(self):
        # Builds a cdf, but defined only from zero to the maximum value in the support
    ...
    def sample(self, r):
        # Given a number 0 <= r <= 1, returns Finv(r). Can be used to sample from this
        #    distribution if r sampled from Uniform(0, 1) RV. Bisection search!
        left  = 0
        right = self.__N  - 1 
